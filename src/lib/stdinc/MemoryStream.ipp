...
inline const void * MemoryOStream::retrieve( int nBytes )
{
	...
	JG_ASSERT( pRead_ + nBytes <= pCurr_ );

	if( !(pRead_ + nBytes <= pCurr_ ) )
	{
		error_ = true;
...
{
	IF_NOT_JG_ASSERT_DEV( pRead_ < pCurr_ )
	{
		error_ = true;
		return -1;
	}
...
inline const void * MemoryIStream::retrieve( int nBytes )
{
	...
	// want machined to JG_ASSERT on bogus packets
 ...
 */
inline
bool MessageStream::addToStream( BufferOStream & stream, uint8 messageID )
{
	int size = this->size();
 ...
 */
inline bool MessageStream::getMessage( BufferIStream & stream,
		int & messageID, int & length )
{
	this->reset();

	IF_NOT_JG_ASSERT_DEV( length >= 2 )
	{
		// Length is too short
		return false;
	}
	...
	messageID = id;
 	...
 	IF_NOT_JG_ASSERT_DEV( length >= (int)size )
	{
		return false;
	}
